...
LPのトップに大きく表示されるファーストビュー画像は、以下の手順で**自由に差し替えられます**：

1. GitHub上で `assets/img/fv/` ディレクトリを開く
2. 「Add file」→「Upload files」を選択
3. お手持ちの画像を **`hero.jpg`** というファイル名でアップロード
4. Commit すれば即座にLPに反映されます

詳細は [`assets/img/fv/README.md`](assets/img/fv/README.md) を参照してください。
画像が未配置の場合は、フォールバック用のSVGプレースホルダーが表示されます。
