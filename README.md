# NAORU整体 渋谷院 リスティング広告用LP プロジェクト
...
### 画像の差し替え方法（ユーザー操作のみで完結）

LP内の主要な画像はHTML/CSSを編集することなく、GitHub上でファイルをアップロードするだけで差し替えられます。

| 配置場所 | 差し替えるファイル名 | 詳細手順 |
|---|---|---|
| **ファーストビュー（全幅画像）** | `assets/img/fv/hero.jpg` | [`assets/img/fv/README.md`](assets/img/fv/README.md) |
| **共感セクション 背景画像** | `assets/img/worries/bg.jpg` | [`assets/img/worries/README.md`](assets/img/worries/README.md) |
| **放置リスクセクション 背景画像** | `assets/img/risks/bg.jpg` | [`assets/img/risks/README.md`](assets/img/risks/README.md) |
| **独自アプローチ 3枚の画像** | `assets/img/approach/reason1〜3.jpg` | [`assets/img/approach/README.md`](assets/img/approach/README.md) |

**共通の手順**：

1. GitHub上で対象ディレクトリを開く
2. 「Add file」→「Upload files」を選択
3. お手持ちの画像を **指定のファイル名**（上記表参照）でアップロード
4. Commit すれば即座にLPに反映されます

画像が未配置の場合は、フォールバック用のSVGプレースホルダーが自動表示されます。
